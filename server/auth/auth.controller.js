...
const register = async (req, res, next) => {
  try {
    ...
    if (role === "admin" && email) {
      ...
      const accessToken = await signAccessToken(userProfile.id);
      const refreshToken = await signRefreshToken(userProfile.id);
      return res.send({ accessToken, refreshToken });
    } else {
