...
const Commission = () => {
  ...
  return (
    <Box className="w-full gap-6">
      ...
      <MathJaxContext>
        <div>
          <Typography variant="h6">Dưới đây là công thức tính phần trăm hoa hồng:</Typography>
          <MathJax>
            {`\\[ \\text{Commission} = \\frac{N \\times (1 - \\frac{S}{100})^{n-1}}{100} \\times \\left(1 + \\frac{P}{100}\\right) \\times \\text{Amount} \\]`}
          </MathJax>

          <Typography variant="h6">Trong công thức này, các ký hiệu có nghĩa như sau:</Typography>
          <ul>
            <li>
              <MathJax inline>{`\\( N \\)`}</MathJax>: Phần trăm hoa hồng mặc định.
            </li>
            <li>
              <MathJax inline>{`\\( S \\)`}</MathJax>: Phần trăm giảm hoa hồng giữa các cấp.
            </li>
            <li>
              <MathJax inline>{`\\( n \\)`}</MathJax>: Cấp bậc hiện tại của người đăng ký, xác định vị trí của họ trong cây đa cấp <i>(n càng lớn càng về đỉnh, n càng nhỏ càng cận khách hàng hiện tại)</i>.
            </li>
            <li>
              <MathJax inline>{`\\( P \\)`}</MathJax>: Phần trăm hoa hồng bổ sung dựa trên hạng của khách hàng, khuyến khích người tham gia phát triển mạng lưới khách hàng.
            </li>
          </ul>
        </div>
      </MathJaxContext>
      ...
      <Grid2 container fullWidth spacing={1} sx={{ mt: 2 }}>
        <Grid2 size={12}>
          <Typography variant="h6">{t("Các thông tin có thể cấu hình")}</Typography>
        </Grid2>
        <Grid2 size={6}>
          <TextField
            label={t("N")}
